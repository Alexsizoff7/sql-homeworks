...
SELECT first_name || ' ' || last_name as Name, job_title, start_date, end_date
FROM employees
...
SELECT employee_id, first_name || ' ' || last_name as Name, job_title, job_id
FROM employees
         JOIN jobs USING (job_id)
WHERE salary <
      (SELECT min_salary
